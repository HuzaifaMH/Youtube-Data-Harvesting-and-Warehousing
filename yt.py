...
st.markdown(f'<h1 style="text-align: center;">Youtube Data Harvesting and Warehousing</h1>', unsafe_allow_html=True)
def Query_section():
    #SQL Connection
    mydb=psycopg2.connect(host="localhost",
                            user='postgres',
                            ...
                            port='5432')
    cursor=mydb.cursor()
    question=st.selectbox("Select Your Question",("Select one",
...
st.write('-> MongoDB')
st.write('-> Data Management using MongoDB (Atlas) and SQL')
st.write('-> Streamlit and API integration')

st.subheader('Approach:')
st.write('1. Set up a Streamlit app: Streamlit is a great choice for building data visualization and analysis tools quickly and easily. You can use Streamlit to create a simple UI where users can enter a YouTube channel ID, view the channel details, and select channels to migrate to the data warehouse.')
st.write('2. Connect to the YouTube API: You will need to use the YouTube API to retrieve channel and video data. You can use the Google API client library for Python to make requests to the API.')
st.write('3. Store data in a MongoDB data lake: Once you retrieve the data from the YouTube API, you can store it in a MongoDB data lake. MongoDB is a great choice for a data lake because it can handle unstructured and semi-structured data easily.')
st.write('4. Migrate data to a SQL data warehouse: After you have collected data for multiple channels, you can migrate it to a SQL data warehouse. You can use a SQL database such as MySQL or PostgreSQL for this.')
st.write('5. Query the SQL data warehouse: You can use SQL queries to join the tables in the SQL data warehouse and retrieve data for specific channels based on user input. You can use a Python SQL library such as SQLAlchemy to interact with the SQL database.')
st.write('6. Display data in the Streamlit app: Finally, you can display the retrieved data in the Streamlit app. You can use Streamlit\'s data visualization features to create charts and graphs to help users analyze the data')
...
st.code('1 - Data Overview')
st.code('2 - Load Data in MongoDB')
st.code('3 - Migrating Data to SQL Database')
st.code('4 - Query Section')
st.code('5 - Exit')

list_options = ['Select one', 'Data Overview', 'Load Data in MongoDB',
                'Migrating Data to SQL Database','Query Section','Exit']
option = st.selectbox('', list_options)
st.write('')
if option:
    if option=='Data Overview':
        show_channels_table()
    elif option=='Load Data in MongoDB':
        ...
        Table=tables()
        st.success(Table)
        channels,playlists,videos,comments = st.tabs(['CHANNELS', 'PLAYLISTS','VIDEOS','COMMENTS'])
        with channels:  
            show_channels_table()      
        with playlists:  
